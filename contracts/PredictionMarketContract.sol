...
contract PredictionMarket is Ownable, Pausable, ReentrancyGuard {
    ...
    function betBear(
        ...
        moch.transferFrom(msg.sender, address(this), _amount);
    ...
    function claim(
        ...
        if (reward > 0) {
            moch.transfer(msg.sender, reward);
        }
    }
    ...
    function claimTreasury() external nonReentrant onlyAdmin {
        ...
        treasuryAmount = 0;
        moch.transfer(adminAddress, currentTreasuryAmount);
    ...
    function recoverToken(address _token, uint256 _amount) external onlyOwner {
        require(_token != address(moch), "Cannot be prediction token address");
        MOCH(payable(_token)).transfer(address(msg.sender), _amount);
